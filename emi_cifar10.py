        ...
        if i==0:
            ...
            for epoch in range(1):
                ...
                for batch_idx, (x, y) in enumerate(train_loader):
                    ...
                    s_feat, s_sim, f_feat, f_sim = Basic_model.forward(images_pair, return_all=True)
                ##########dmi##########
                    simclr = torch.cat([s_sim[:x.shape[0]], s_sim[images1.shape[0]:][:x.shape[0]]], dim=0)
                    simclr = normalize(s_sim)  # normalize
                    
                    feature_map_out= torch.cat([s_feat[:x.shape[0]], s_feat[images1.shape[0]:][:x.shape[0]]], dim=0)
                    feature_map_out = normalize(s_feat[:images_pair.shape[0]]) 
                    ...
                    cross_task_sim, _ = Supervised_NT_xent_simn(sim_matrix, labels=rot_sim_labels,
                                                   temperature=0.07,miu=miun)
                
                    dmi = cross_task_sim * 1

                #########dmi and rmi################

                    simclr = normalize(f_sim)  # normalize
                    feature_map_out = normalize(f_feat[:images_pair.shape[0]])
                   # num1 = feature_map_out.shape[1] // simclr.shape[1]
                    num1 = feature_map_out.shape[1] - simclr.shape[1]
                    id1 = torch.randperm(num1)[0]
                    size = simclr.shape[1]
                    ...
                    rmi = Supervised_NT_xent_proto(new_sim_proto, labels1=torch.cat([y, y],dim=0), labels2=new_proto_label, temperature=0.07)
                    loss_pp_new =Supervised_NT_xent_pp(cur_new_proto_z, cur_new_proto_zt)
                    smi = loss_pp_new + loss_pp_buf   
                    miloss = smi+rmi + dmi
                    loss = ce+1*lo1 + miloss
            ...
            #    S_model[i].train()
                ...
                for batch_idx, (x, y) in tqdm(enumerate(train_loader)):
                    ...
                    s_feat, s_sim, f_feat, f_sim = Basic_model.forward(t, return_all=True)
                    ###########################dmi##################
                    slow_faet = normalize(s_feat[:images_pair.shape[0]])
                    slow_faet_r = normalize(s_feat[images_pair.shape[0]:])

                    slow_sim = s_sim[:images_pair.shape[0]]
                    slow_sim_r = s_sim[images_pair.shape[0]:]
                    slow_sim = normalize(slow_sim)
                    slow_sim_r = normalize(slow_sim_r)

                    num1 = slow_faet.shape[1] - slow_sim.shape[1]
                    id1 = torch.randperm(num1)[0]

                    id2=torch.randperm(num1)[0]

                    size = slow_sim.shape[1]

                    sim_matrix = torch.matmul(slow_sim, slow_faet[:, id1:id1 + size].t())
                    sim_matrix_r = torch.matmul(slow_sim_r,
                                                     slow_faet_r[:, id2:id2 + size].t())

                    sim_matrix += 1 * get_similarity_matrix(slow_sim) 
                    sim_matrix_r += 1 * get_similarity_matrix(slow_sim_r)

                    sim_n, simn_mask =Supervised_NT_xent_simn(sim_matrix,labels=rot_sim_labels,temperature=0.07, miu=miun)
                    sim_b, simb_mask = Supervised_NT_xent_simb(sim_matrix_r, labels=rot_sim_labels_r, temperature=0.07, miu=miub)
                    dmi = sim_b + sim_n
                    #################################ocm loss###########################
                 ...
                 #################################rmi and smi###########################
                    ...
                    bx_for_proto, by_for_proto=sample_similar_for_proto(mem_x, mem_y, buffero, 6, class_x_list, class_y_list, class_id_map)
                    ...
                    z_buf=  torch.cat([zr[: mem_x.shape[0]], zrt[: mem_x.shape[0]]], dim=0)
                    ...
                    loss_sim_proto_buffer = Supervised_NT_xent_proto(buffer_sim_proto, labels1=by_proto_label, labels2=torch.cat([mem_y, mem_y],dim=0), temperature=0.07)
